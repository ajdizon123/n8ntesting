...
export class CharitysPurseDonationConfirmed implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'CharitysPurse Donation Confirmed',
		name: 'charitysPurseDonationConfirmed',
		group: ['trigger'],
		version: 15,
		description: 'Triggers when a donation is confirmed',
		defaults: { name: 'Donation Confirmed Trigger' },
		inputs: [],
	...
	/**
	 * Poll-based trigger for confirmed donations
	 */
	async poll(this: IPollFunctions): Promise<INodeExecutionData[][] | null> {
		...
		const filteredDonations = donationsArray.filter((d: any) => {
			// Only look for confirmed donations
			const statusMatch = d?.status === 'confirmed';
			const id = d?._id;
